...
func physics_update(delta: float) -> void:
	...
	if Flags.is_enabled(Flags.DEBUG_NAV_AGENT):
		...
		DebugDraw.set_text("player_position", character.global_position, character.global_position)
		DebugDraw.set_text("next_position", next_position, next_position)
	...
	if character.is_facing(next_position, 0.9):
